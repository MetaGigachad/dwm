...
static int statusbartext_width = 0;
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	while (text[++i]) {
		if (text[i] == '^' && !isCode) {
			...
			text[i] = '\0';
			w = TEXTW(text) - lrpad;
			drw_text(drw, x, statusvertpad, w, bh - 2 * statusvertpad, 0, text, 0);
...
setlayout(const Arg *arg)
{
	...
	if (selmon->sel)
		...
		drawbar(selmon);
    if (strcmp(((const Layout*)arg->v)->symbol, "[M]") == 0) {
        selmon->showbar = selmon->pertag->showbars[selmon->pertag->curtag] = 0;
    } else {
...
updatesystray(void)
{
	...
	unsigned int sw = statusbartext_width - lrpad + systrayspacing;
	unsigned int w = 1;
